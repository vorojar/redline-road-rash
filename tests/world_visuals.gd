...
func run() -> void:
	...
	check(face.mesh.size.x==12 and is_equal_approx(face.mesh.size.y,12.0*809/1942) and face.position.z>.19,"易哈佛高架广告牌保持原图比例且面板不与背板重叠")
	check(face.material_override.albedo_texture.resource_path=="res://assets/textures/sponsors/ehafo_medical.png" and face.material_override.albedo_texture.get_image().has_mipmaps(),"路牌使用用户医护原图与 mipmaps")
	check(billboard.find_children("*","CollisionObject3D",true,false).is_empty(),"路外广告牌不新增游戏碰撞障碍")
	billboard.queue_free()
	var highway_board = Details.sponsor_board("OpenAI")
	...
	var leosto_board = Details.sponsor_board("LEOSTO")
	root.add_child(leosto_board)
	var leosto_face = leosto_board.get_node("Sponsor Face")
	check(leosto_face.mesh.size==Vector2(12,5),"LEOSTO 广告画面为12×5米")
	check(leosto_face.material_override.albedo_texture.resource_path=="res://assets/textures/sponsors/leosto.png" and leosto_face.material_override.albedo_texture.get_image().has_mipmaps(),"LEOSTO 正面贴图和 mipmaps")
	leosto_board.queue_free()
	for brand in ["NVIDIA","Microsoft","Apple","Amazon","Anthropic","Gemini","Tesla"]:
		var board = Details.sponsor_board(brand)
		root.add_child(board)
		var brand_face = board.get_node("Sponsor Face")
		check(brand_face.mesh.size==Vector2(12,5),brand+" 使用 12×5 米高架画面")
		check(brand_face.material_override.albedo_texture.resource_path=="res://assets/textures/sponsors/"+brand.to_lower()+".png" and brand_face.material_override.albedo_texture.get_image().has_mipmaps(),brand+" 贴图正确且具有实际 mipmaps")
		check(board.find_children("*","CollisionObject3D",true,false).is_empty(),brand+" 不新增路外碰撞")
		board.queue_free()
