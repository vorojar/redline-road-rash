...
const KEYS = {"throttle":KEY_W,"brake":KEY_S,"left":KEY_A,"right":KEY_D,"punch":KEY_J,"kick":KEY_K,"club":KEY_L,"guard":KEY_U,"dodge":KEY_I,"grab":KEY_O,"boost":KEY_SHIFT,"cruise":KEY_SPACE,"pause":KEY_ESCAPE,"restart":KEY_R}
...
static func setup(bindings: Dictionary) -> void:
	for action in KEYS:
		...
		key.physical_keycode = int(bindings.get(action, KEYS[action]))
...
static func key_label(action: String, bindings: Dictionary) -> String:
	return OS.get_keycode_string(int(bindings.get(action, KEYS[action])))
